...
if(!all(countNA==".")){
  stats_estim <- cbind(stats_estim,countNA)[,c("target_values","mean","meanBoot","bias","biasBoot","sd_emp","sd_hess","countNA","sd_boot","cover_hess","cover_boot")]
}
...
options(scipen=999)
stats_estim[,-1] <- sapply(stats_estim[,-1], FUN=function(x){as.character(sapply(x,FUN=function(y){signif(y,precision)}))})
...
stats_estimLTX <- cbind(Parameters=tex2_names,stats_estim) %>% flextable() %>%
  ...
  add_header_lines(paste0("Results of estimation, with bootstrap, for Irene model with",if(data=="Ab_G1-5"){" 5 genes"}," using ",nBoot," bootstraps and ",nrep," replicates.\n")) %>%
...
#stats_estimLTX <- set_escape_contents(stats_estimLTX,FALSE)
...
# print(xtable::xtable(stats_estimLTX),sanitize.text.function=function(x){x})
